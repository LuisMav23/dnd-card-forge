...
export default function BackgroundTextureSection({ currentTexture, onTextureChange }: Props) {
  ...
  useEffect(() => {
    if (currentTexture) drawPreview(currentTexture);
  }, [currentTexture, drawPreview]);

  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    const previousUrl = currentTexture;
    setUploading(true);
    try {
      const blob = await processFileToBlob(file);
      if (!blob) throw new Error('Could not process image');
      const url = await uploadUserAsset(blob, 'card-texture');
      onTextureChange(url);
    ...
    <div className="fsec">
      ...
      <div className="frow c1">
        <div className="fg">
          ...
          <div className={`prev-wrap${currentTexture ? ' vis' : ''}`}>
