...
export default function ThemeToggle({ variant = 'icon', onMenuThemeChange }: ThemeToggleProps) {
  ...
  return (
    <button
      ...
      onClick={next}
      className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-bdr-2 bg-panel/90 text-gold-dark transition-colors hover:border-gold-dark hover:bg-mid hover:text-gold dark:border-bdr dark:text-gold dark:hover:text-gold-light"
    >
      {isDark ? (
