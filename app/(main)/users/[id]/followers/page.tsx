...
export default function UserFollowersPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/users/${id}/followers?limit=50`, { cache: 'no-store' });
  ...
  return (
    <div className="page-radial-soft flex min-h-0 flex-1 flex-col bg-bg">
      <main className="mx-auto w-full max-w-lg flex-1 px-4 py-8 sm:px-6">
        ...
        <h1 className="mt-6 font-[var(--font-cinzel),serif] text-xl font-bold text-gold">Followers</h1>
        {loading && <p className="mt-6 text-sm text-muted">Loading…</p>}
        {error && <p className="mt-6 text-sm text-red-300">{error}</p>}
        {!loading && !error && users.length === 0 && (
          <p className="mt-6 text-sm text-bronze">No followers yet.</p>
        )}
