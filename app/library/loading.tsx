...
export default function LibraryLoading() {
  return (
    <div className="page-radial-soft flex min-h-screen flex-col bg-bg">
      <Header />
      <main
        className="animate-fade-in flex min-h-0 flex-1 flex-col overflow-y-auto px-4 py-6 sm:px-8 sm:py-10"
        role="status"
        aria-busy="true"
          ...
          <section className="flex min-h-0 min-w-0 flex-1 flex-col gap-4 lg:pl-6">
            ...
            <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <li
                  key={i}
                  className="flex flex-col rounded-xl border border-bdr bg-gradient-to-b from-panel to-mid/95 p-5 shadow-[0_8px_32px_rgba(0,0,0,0.35)] sm:p-6"
                >
