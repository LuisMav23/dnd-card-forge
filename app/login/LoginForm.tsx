...
async function postLoginDestination(): Promise<'/onboarding' | '/home'> {
  ...
  if (
    profRes.ok &&
    profData.profile &&
    !profData.profile.onboarding_completed_at
  ) {
    return '/onboarding';
  }
  return '/home';
...
export default function LoginForm() {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      const dest = await postLoginDestination();
      router.push(dest);
      router.refresh();
    } catch (err: unknown) {
      ...
      <div className="relative mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
        <div className="rounded-xl border border-bdr bg-panel/95 p-8 shadow-[0_0_0_1px_rgba(201,168,76,0.08),0_24px_48px_rgba(0,0,0,0.55)] backdrop-blur-sm sm:p-10">
          <div className="mb-8 text-center">
            <Link
              href="/"
              className="mx-auto mb-3 flex justify-center text-gold-dark transition-opacity hover:opacity-90 dark:text-gold"
