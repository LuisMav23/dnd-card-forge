...
export const CARD_TYPE_ORDER: { type: CardType; emoji: string; label: string }[] = [
  { type: 'spell', emoji: '✨', label: 'Spell' },
  { type: 'armor', emoji: '🛡', label: 'Armor' },
  { type: 'equipment', emoji: '🎒', label: 'Equipment' },
  { type: 'weapon', emoji: '⚔️', label: 'Weapon' },
  { type: 'sidekick', emoji: '🧑', label: 'Sidekick' },
...
export function getTypebar(cardType: CardType, fields: Record<string, string>): { left: string; right: string } {
  ...
  switch (cardType) {
    case 'spell': {
      ...
      const sn = sch.split(' ').slice(1).join(' ');
      return { left: `${t} — ${sn} · ${a}`, right: sch };
    }
    case 'armor': {
    ...
    case 'weapon': {
      ...
      return { left: `${w} — ${d} · ${p}`, right: '⚔️ Weapon' };
    }
    case 'sidekick': {
